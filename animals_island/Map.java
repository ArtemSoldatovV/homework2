    ...
    public void bypassing_sthe_map(){
        for (int i=0; i < mas.length;i++){
            ...
            for (int i2=0; i2 < mas[0].length;i2++){
                ...
                for (int i3 =0;i<mas[i][i2].get_List_Herbivores().size();i++){
                    mas[i][i2] = n.get(i3).eat(mas[i][i2]); // есть
                    mas[i][i2] = n.get(i3).reproduce(mas[i][i2],i3);// расмножаться
                    mas[i][i2] = n.get(i3).die(mas[i][i2],i3);// проверить существо
                    mas = n.get(i3).movement_animals(mas,i,i2,i3);// переместится
                }
            }
